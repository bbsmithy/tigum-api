...
pub fn delete_code(conn: &TigumPgConn, id: i32) -> Json<String> {
    let update = conn
        .execute("DELETE FROM code WHERE id = $1", &[&id])
        .unwrap();
    Json(format!("{} rows affected", update))
}

pub fn update_code(conn: &TigumPgConn, id: i32, code: Json<NewCode>) -> Json<Code> {
    let updated_rows = conn.query(
        "UPDATE code SET topic_id = $2, user_id = $3, content = $4, origin = $5, language = $6 WHERE id = $1 RETURNING *",
        &[&id, &code.topic_id, &code.user_id, &code.content, &code.origin, &code.language],
    ).unwrap();
...
pub fn get_codes(conn: &TigumPgConn, ids: Json<Ids>) -> Json<Vec<Code>> {
    println!("{:?}", ids);
    let query_result = conn
        .query("SELECT * FROM code WHERE id = ANY($1)", &[&ids.ids])
        .unwrap();
...
pub fn get_code(conn: &TigumPgConn, id: i32) -> Json<Code> {
    let query_result = conn
        .query("SELECT * FROM code WHERE id = $1", &[&id])
        .unwrap();
...
pub fn create_code(conn: &TigumPgConn, code: &Json<NewCode>) -> Json<Id> {

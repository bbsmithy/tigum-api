...
pub fn get_user(conn: &TigumPgConn, email_hash: u64) -> ApiResponse {
    ApiResponse {
        json: json!("All good"),
...
pub fn update_password(conn: &TigumPgConn, email_hash: i64, password_hash: String) -> ApiResponse {
    ApiResponse {
        json: json!("All good"),
        status: Status::raw(200)
    }
...
pub fn verify_user_with_hash(conn: &TigumPgConn, hash: String) -> bool {
    true
...
pub fn set_user_as_verified(conn: &TigumPgConn, hash: String) -> bool {
    true
    // let check_for_user = conn.run(move |c|
    //     c.query(

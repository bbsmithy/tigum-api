...
pub fn get_public_topics_for_user(conn: &diesel::PgConnection, user_name: String) -> ApiResponse {
    ...
    match check_for_user {
        Ok(row) => {
            let published_topics_res = topics.filter(user_id.eq(row.id)).filter(published.eq(true)).get_results::<Topic>(conn);
...
pub fn get_public_notes_in_topic(conn: &diesel::PgConnection, notes_topic_id: i32) -> ApiResponse {
    ...
    let published_notes_res = notes.filter(topic_id.eq(notes_topic_id)).filter(published.eq(true)).get_results::<Note>(conn);
...
pub fn get_public_videos_in_topic(conn: &diesel::PgConnection, videos_topic_id: i32) -> ApiResponse {
    ...
    let published_videos_res = videos.filter(topic_id.eq(videos_topic_id)).filter(published.eq(true)).get_results::<Video>(conn);
...
pub fn get_public_snippets_in_topic(conn: &diesel::PgConnection, snippet_topic_id: i32) -> ApiResponse {
    ...
    let published_videos_res = article_snippets.filter(topic_id.eq(snippet_topic_id)).filter(published.eq(true)).get_results::<ArticleSnippet>(conn);
    if let Ok(result) = published_videos_res {
        ApiResponse {
            status: Status::raw(200),
...
pub fn get_public_links_in_topic(conn: &diesel::PgConnection, link_topic_id: i32) -> ApiResponse {
    ...
    let published_videos_res = links.filter(topic_id.eq(link_topic_id)).filter(published.eq(true)).get_results::<Link>(conn);
    if let Ok(result) = published_videos_res {
        ApiResponse {
            status: Status::raw(200),
